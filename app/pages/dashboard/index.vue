...
<template>
  <div class="space-y-4">
    ...
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
      <DashboardKpiTile label="MRR" :value="`$${formatK(totalMrr)}`" :delta="kpi.mrr.delta" :icon="DollarSign" icon-class="text-muted-foreground">
        <Sparkline :data="kpi.spark.revenue" :height="36" class="mt-1.5" />
      </DashboardKpiTile>

      <DashboardKpiTile label="Active users" value="12,847" :delta="kpi.users.delta" :icon="Users" icon-class="text-muted-foreground">
        <BarChart :data="kpi.spark.users.map((v, i) => ({ x: i, y: v }))" :height="36" :option="miniChrome" class="mt-1.5" />
      </DashboardKpiTile>

      <DashboardKpiTile label="Requests / min" value="2,484" :delta="kpi.rpm.delta" :icon="Zap" icon-class="text-muted-foreground">
        <AreaChart :data="kpi.spark.requests.map((v, i) => ({ x: i, y: v }))" :height="36" :option="miniChrome" class="mt-1.5" />
      </DashboardKpiTile>

      <DashboardKpiTile label="Conversion" value="7.4%" :delta="kpi.conversion.delta" :icon="TrendingUp" icon-class="text-muted-foreground">
        <BarChart :data="kpi.spark.conversion.map((v, i) => ({ x: i, y: v }))" :height="36" :option="miniChrome" class="mt-1.5" />
      </DashboardKpiTile>

      <!-- Avg latency: rising is bad, so delta tone is negative. -->
      <DashboardKpiTile label="Avg latency" value="412ms" :delta="kpi.latency.delta" delta-tone="negative" :icon="Zap" icon-class="text-muted-foreground">
        <AreaChart :data="kpi.spark.latency.map((v, i) => ({ x: i, y: v }))" :height="36" :option="miniChrome" class="mt-1.5" />
      </DashboardKpiTile>

      <!-- Churn: down is good, so delta stays positive even though it's
           a negative number. -->
      <DashboardKpiTile label="Churn" value="1.8%" :delta="kpi.churn.delta" :icon="TrendingDown" icon-class="text-muted-foreground">
      ...
      <Card class="lg:col-span-2">
        <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-3">
          <div>
            ...
            <CardDescription class="text-xs">{{ rangeLabel }} · in USD</CardDescription>
          </div>
          <Badge variant="outline" class="text-[10px]">MRR {{ kpi.mrr.delta }}</Badge>
        </CardHeader>
        <CardContent class="pb-3">
          <LineChart :data="revenueSeries" x-field="x" :y-field="['revenue', 'expenses']" :height="300" />
        </CardContent>
      </Card>
      <Card class="lg:col-span-2">
        <CardHeader class="pb-3">
          <CardTitle class="text-sm">Conversion funnel</CardTitle>
          <CardDescription class="text-xs">{{ rangeLabel }} · 1.8% end-to-end (60% → 40% → 30% → 25%)</CardDescription>
        </CardHeader>
        <CardContent class="pb-3">
    ...
    <div class="grid gap-3 lg:grid-cols-6">
      <Card class="lg:col-span-2">
        <CardHeader class="pb-3">
          <CardTitle class="text-sm">{{ requestsBlock.title }}</CardTitle>
          <CardDescription class="text-xs">{{ requestsBlock.subtitle }}</CardDescription>
        </CardHeader>
        <CardContent class="pb-3">
          <BarChart :data="requestsBlock.data" x-field="x" y-field="y" :height="200" />
        </CardContent>
      </Card>
      <Card class="lg:col-span-2">
        <CardHeader class="pb-3">
    ...
    <Card>
      <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
        <div>
          ...
          <CardDescription class="text-xs">{{ totalDeploys.toLocaleString() }} deploys · longest streak 18 days</CardDescription>
        </div>
    ...
    <div class="grid gap-3 lg:grid-cols-6">
      ...
      <SectionCard title="Recent activity" description="Live feed across products" class="lg:col-span-2">
        <DataList>
          <DataListItem v-for="(item, i) in activities" :key="i">
            <div class="flex items-center gap-2.5">
              <IconBox :icon="item.icon" variant="muted" :icon-class="item.iconClass" />
              <div>
                <p class="text-[13px] font-medium">{{ item.title }}</p>
                <p class="text-muted-foreground text-[11px]">{{ item.detail }}</p>
              </div>
            </div>
