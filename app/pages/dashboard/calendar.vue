  ...
  <div class="flex flex-col gap-5" @mouseup="endDrag">
    ...
    <header class="flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        <h1 class="text-2xl font-semibold tracking-tight mt-2">Calendar</h1>
        <p class="text-muted-foreground text-sm">Schedule, meetings, and deadlines. Drag or shift-click to select a range.</p>
      </div>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <!-- Watermark icon -->
        <component :is="meta.icon" :class="['pointer-events-none absolute -right-4 -top-4 size-24 opacity-[0.06] transition-opacity group-hover:opacity-[0.12]', meta.text]" />
        <!-- Soft glow blob -->
        ...
        <div class="flex items-center justify-between gap-2">
          <div class="flex items-center gap-1.5">
            ...
            <p class="text-muted-foreground text-[10px] uppercase tracking-[0.14em] font-medium">{{ meta.label }}</p>
          </div>
        ...
        <div class="mt-2 flex items-baseline gap-1.5">
          <p class="text-3xl font-semibold tabular-nums leading-none">{{ monthCounts[type] }}</p>
        ...
        <div class="mt-3 border-t border-border/50 pt-2 min-h-[2.25rem]">
          <template v-if="typeStats[type].next">
            ...
            <p class="truncate text-xs font-medium mt-0.5">{{ typeStats[type].next!.title }}</p>
          </template>
          <template v-else>
            <p class="text-[10px] uppercase tracking-wider text-muted-foreground">No upcoming</p>
            <p class="text-xs text-muted-foreground/70 mt-0.5">All clear</p>
          </template>
        </div>
      </button>
    </div>
    ...
    <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <div class="rounded-xl border bg-card/40 overflow-hidden">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 border-b bg-muted/30 px-4 py-2.5">
          <div class="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" class="h-7 text-xs" @click="goToday">Today</Button>
            <h2 class="text-sm font-semibold ml-2">{{ monthLabel }}</h2>
          </div>
          <div class="flex items-center gap-3 text-[11px] text-muted-foreground">
            <div v-if="isRange" class="flex items-center gap-1.5 rounded-full bg-primary/10 px-2 py-0.5 text-primary ring-1 ring-inset ring-primary/20">
        ...
        <div class="grid grid-cols-7 select-none">
          <template v-if="loading">
            <div v-for="i in 35" :key="i" class="h-24 border-b border-r p-1.5 last:border-r-0">
          ...
          <template v-else>
            <ContextMenu v-for="(d, i) in gridDays" :key="d.key">
              <ContextMenuTrigger as-child>
                ...
                >
                  <div class="flex items-center justify-between">
                    ...
                    <span v-if="(eventsByDate.get(d.key)?.length ?? 0) > 0" class="text-[9px] tabular-nums text-muted-foreground">
                  ...
                  <div class="flex flex-col gap-0.5">
                    <ContextMenu v-for="e in (eventsByDate.get(d.key) ?? []).slice(0, 2)" :key="e.id">
                      ...
                      <ContextMenuContent class="w-52">
                        ...
                        <ContextMenuItem class="gap-2"><Pencil class="size-3.5" /> Edit</ContextMenuItem>
                        <ContextMenuItem class="gap-2"><CopyPlus class="size-3.5" /> Duplicate</ContextMenuItem>
                    ...
                    <span v-if="(eventsByDate.get(d.key)?.length ?? 0) > 2" class="px-1 text-[10px] text-muted-foreground">
              ...
              <ContextMenuContent class="w-56">
                <ContextMenuLabel class="text-[11px] text-muted-foreground">{{ fmtDayLong(d.key) }}</ContextMenuLabel>
                <ContextMenuSeparator />
                ...
                <ContextMenuItem class="gap-2" :disabled="(eventsByDate.get(d.key)?.length ?? 0) === 0" @select="rangeAnchor = d.key; rangeStart = d.key; rangeEnd = d.key">
                ...
                <ContextMenuItem class="gap-2" @select="copyDate(d.key)">
                  <Copy class="size-3.5" /> Copy date <ContextMenuShortcut class="tabular-nums">{{ d.key }}</ContextMenuShortcut>
                </ContextMenuItem>
                <ContextMenuItem class="gap-2" @select="goToday">
                  <ArrowRight class="size-3.5" /> Go to today
                </ContextMenuItem>
                <ContextMenuItem v-if="isRange" class="gap-2 text-destructive focus:text-destructive" @select="clearRange">
        ...
        <div class="flex flex-wrap items-center gap-3 border-t bg-muted/20 px-4 py-2 text-[10px] text-muted-foreground">
          ...
          <div v-for="(meta, key) in typeMeta" :key="key" class="flex items-center gap-1.5">
      ...
      <aside class="flex flex-col gap-4">
        ...
        <div v-if="!isRange" class="rounded-xl border bg-card/40 overflow-hidden">
          <div class="border-b bg-muted/30 px-4 py-3">
            <div class="flex items-center justify-between">
              ...
              <div class="text-right">
                <p class="text-[10px] uppercase tracking-wider text-muted-foreground">Events</p>
          ...
          <div class="max-h-[420px] overflow-y-auto p-3">
            ...
            <template v-else-if="selectedDayEvents.length === 0">
              <div class="flex flex-col items-center justify-center gap-2 py-8 text-center">
                ...
                <Button size="sm" variant="outline" class="mt-1 gap-1.5 h-7 text-xs">
                  ...
                  >
                    ...
                    <div class="min-w-0 flex-1 space-y-1">
                      ...
                      <div class="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-muted-foreground">
                        ...
                        <span v-if="e.location" class="inline-flex items-center gap-1"><MapPin class="size-3" />{{ e.location }}</span>
                      </div>
                      <div v-if="e.attendees?.length" class="flex items-center -space-x-1.5 pt-0.5">
                        <Avatar v-for="(a, i) in e.attendees.slice(0, 4)" :key="i" class="size-5 border-2 border-background">
                          <AvatarFallback class="text-[8px] bg-primary/10 text-primary">{{ initials(a) }}</AvatarFallback>
                        </Avatar>
                        <span v-if="e.attendees.length > 4" class="pl-2 text-[10px] text-muted-foreground">+{{ e.attendees.length - 4 }}</span>
                      </div>
                    </div>
                  </div>
                </ContextMenuTrigger>
                <ContextMenuContent class="w-48">
                  <ContextMenuLabel class="text-[11px] text-muted-foreground truncate">{{ e.title }}</ContextMenuLabel>
                  <ContextMenuSeparator />
                  <ContextMenuItem class="gap-2" @select="openEvent(e)"><Eye class="size-3.5" /> View details</ContextMenuItem>
                  <ContextMenuItem class="gap-2"><Pencil class="size-3.5" /> Edit</ContextMenuItem>
                  <ContextMenuItem class="gap-2"><CopyPlus class="size-3.5" /> Duplicate</ContextMenuItem>
                  <ContextMenuSeparator />
                  <ContextMenuItem class="gap-2 text-destructive focus:text-destructive"><Trash2 class="size-3.5" /> Cancel event</ContextMenuItem>
                </ContextMenuContent>
              </ContextMenu>
            </template>
          </div>
        ...
        <!-- Range summary -->
        <div v-else class="rounded-xl border bg-card/40 overflow-hidden">
          <div class="border-b bg-gradient-to-r from-primary/10 to-transparent px-4 py-3">
            <div class="flex items-center justify-between gap-3">
              <div class="min-w-0">
                <p class="text-[10px] uppercase tracking-wider text-muted-foreground">Range · {{ rangeDayCount }} days</p>
            ...
            <div class="mt-3 grid grid-cols-4 gap-2">
              <div v-for="(meta, t) in typeMeta" :key="t" class="rounded-md border bg-background/40 px-2 py-1.5">
          ...
          <div class="max-h-[420px] overflow-y-auto">
            <template v-if="rangeEvents.length === 0">
              <p class="px-4 py-8 text-center text-xs text-muted-foreground">No events in range.</p>
            </template>
            <template v-else>
              <div v-for="e in rangeEvents" :key="e.id"
        ...
        <div class="rounded-xl border bg-card/40 overflow-hidden">
          <div class="border-b bg-muted/30 px-4 py-2.5">
            ...
            <p class="text-[11px] text-muted-foreground">After today</p>
          </div>
          <div class="divide-y">
            <template v-if="loading">
            ...
            <template v-else-if="upcoming.length === 0">
              <p class="px-4 py-6 text-center text-xs text-muted-foreground">Nothing on the horizon.</p>
            </template>
            <template v-else>
              <button
                v-for="e in upcoming"
              ...
              >
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p class="text-[11px] text-muted-foreground tabular-nums mt-0.5">{{ e.start }}–{{ e.end }}<span v-if="e.location"> · {{ e.location }}</span></p>
                </div>
              </button>
            </template>
          </div>
        ...
        <DialogHeader>
          <div class="flex items-center gap-2">
            <div :class="['flex size-8 items-center justify-center rounded-md ring-1 ring-inset', typeMeta[selectedEvent.type].chip]">
              <component :is="typeMeta[selectedEvent.type].icon" class="size-4" />
            </div>
            <div>
              <DialogTitle class="text-base">{{ selectedEvent.title }}</DialogTitle>
              <DialogDescription class="text-xs">{{ typeMeta[selectedEvent.type].label }} · {{ selectedEvent.status }}</DialogDescription>
            </div>
          </div>
        </DialogHeader>
          ...
          <div class="grid gap-2 text-sm">
            ...
            <div v-if="selectedEvent.attendees?.length" class="flex items-start gap-2">
              ...
              <div class="flex flex-wrap items-center gap-1.5">
                <div v-for="a in selectedEvent.attendees" :key="a" class="flex items-center gap-1.5 rounded-full bg-muted px-2 py-0.5 text-xs">
                  <Avatar class="size-4">
                    <AvatarFallback class="text-[8px] bg-primary/10 text-primary">{{ initials(a) }}</AvatarFallback>
                  </Avatar>
                  <span>{{ a }}</span>
