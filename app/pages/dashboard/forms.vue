  ...
  <form class="space-y-4" @submit="onSubmit">
    <header class="flex items-end justify-between gap-4">
      <div class="space-y-1">
        ...
        <p class="text-muted-foreground text-sm">Update your profile, notification preferences, and billing.</p>
      </div>
      <div class="flex items-center gap-3">
        <span v-if="savedAt" class="text-muted-foreground inline-flex items-center gap-1 text-xs">
          <Check class="size-3 text-emerald-600" />Saved at {{ savedAt }}
        </span>
        <Button type="submit" size="sm" :disabled="submitting" class="gap-1.5">
    ...
    <Card>
      ...
      <CardContent class="grid gap-4 sm:grid-cols-2">
        <div class="space-y-1.5">
          ...
          <Input id="name" v-model="profile.name" placeholder="Jane Doe" />
        </div>
        <div class="space-y-1.5">
          <Label for="email">Work email</Label>
          <Input id="email" v-model="profile.email" type="email" placeholder="you@company.com" />
        </div>
        <div class="space-y-1.5 sm:col-span-2">
          <Label for="bio">Short bio</Label>
          <Textarea id="bio" v-model="profile.bio" :rows="3" :maxlength="280" placeholder="Tell people what you work on…" />
          <p class="text-muted-foreground text-[11px] tabular-nums">{{ profile.bio.length }} / 280</p>
        </div>
      </CardContent>
    </Card>
    ...
    <Card>
      ...
      <CardContent class="grid gap-4 sm:grid-cols-2">
        <div class="space-y-1.5">
          ...
          <Input id="password" v-model="account.password" type="password" placeholder="Leave blank to keep current" show-password-toggle />
        </div>
        <div class="space-y-1.5">
        ...
        <div class="space-y-2 sm:col-span-2">
          ...
          <RadioGroup v-model="account.visibility" class="sm:grid-cols-3">
